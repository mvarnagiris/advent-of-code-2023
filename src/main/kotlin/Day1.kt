fun day1Part1(): String = getResourceAsText("day1.txt")
    .split(NewLine)
    .sumOfLines("([1-9])".toRegex())

private val wordDigits = listOf("one", "two", "three", "four", "five", "six", "seven", "eight", "nine")
private val wordDigitsRegex = "(?=(${wordDigits.joinToString(separator = "|")}|[1-9]))".toRegex()

fun day1Part2(): String = getResourceAsText("day1.txt")
    .split(NewLine)
    .sumOfLines(wordDigitsRegex)

private fun List<String>.sumOfLines(regex: Regex) = map { line ->
    val matches = regex.findAll(line)
    matches.first().toDigit() + matches.last().toDigit()
}.sumOf { it.toInt() }.toString()

private fun MatchResult.toDigit() = groupValues[1].toDigit()
private fun String.toDigit() = if (length == 1 && get(0).isDigit()) {
    this
} else {
    (wordDigits.indexOf(this) + 1).toString()
}
 ...
 * blank ("you sure ask a lot of questions") and hang on did you just say the sky ("of course, where do you think snow comes from") when you realize that the
